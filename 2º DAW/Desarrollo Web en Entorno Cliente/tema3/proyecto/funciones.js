...
export function listarVideojuegos() {
    ...
    videojuegos.forEach(videojuego => {
        output.innerHTML += `${videojuego.nombre} - Género: ${videojuego.genero}, Año: ${videojuego.lanzamiento}, Puntuación: ${videojuego.puntuacion}<br>`;
    });
}
...
export function buscarPorGenero(genero) {
    ...
    if (filtrados.length > 0) {
        ...
        filtrados.forEach(videojuego => {
            output.innerHTML += `${videojuego.nombre} - Año: ${videojuego.lanzamiento}, Puntuación: ${videojuego.puntuacion}<br>`;
        });
    } else {
        output.innerHTML += `No se encontraron videojuegos del género "${genero}".<br>`;
    }
...
export function ordenarPorPuntuacion() {
    ...
    ordenados.forEach(videojuego => {
        output.innerHTML += `${videojuego.nombre} - Puntuación: ${videojuego.puntuacion}<br>`;
    });
}
